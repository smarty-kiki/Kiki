...
//  kiki-desktop-agent
//
//  PostHog analytics wrapper. Every event name and property is defined here.
...
enum KikiAnalytics {
    ...
    /// The user began onboarding for the first time.
    ...
    /// The 40s onboarding demo interaction where Kiki points at something.
    ...
    /// The reply arrived and is being spoken via TTS.
    ...
    /// The reply carried a [POINT:x,y:label] tag, so the cursor is flying to that element.
