...
//  kiki-desktop-agent
//
//  Push-to-talk dictation: captures microphone audio with AVAudioEngine, routes
//  it into the active transcription provider, and hands back the final draft.
//
...
final class BuddyDictationManager: NSObject, ObservableObject {
    ...
    /// The settings panel uses this to decide whether to show the Speech Recognition
    /// row at all: the network providers transcribe off-device and never ask for it.
    private(set) var transcriptionProviderRequiresSpeechRecognitionPermission = false
    ...
    func requestInitialPushToTalkPermissionsIfNeeded() async {
        ...
        do {
            ...
            // Cancelled while waiting for the app to come forward — safe to
            // continue into the permission check.
        }
    ...
    private func startPushToTalk(
        ...
        if needsInitialPermissionPrompt {
            ...
            do {
                ...
                // Cancelled while the app is being activated — safe to continue
    ...
    private func stopPushToTalk(expectedStartSource: BuddyDictationStartSource) {
        ...
        // One of the three ways to a transcript, and the one that says the
        // recognizer did not answer in time. The other two are the final result
        // and the error path.
        finalizeFallbackWorkItem?.cancel()
        ...
        let activeTranscriptionSession = try await transcriptionProvider.startStreamingSession(
            ...
            onFinalTranscriptReady: { [weak self] transcriptText in
                Task { @MainActor in
                    ...
                    // Only once the key is up: a "final" result while the user is still
                    // holding the key is one segment of an ongoing session rather than
                    // the end of it, and the text is already banked above.
    ...
    private func handleRecognitionError(_ error: Error) {
        ...
        // An error after the key came up is not a failure to transcribe: the
        // recognizer has already reported what it heard, and the session finishes on it.
    ...
    /// macOS can show the microphone/speech sheet a second time if several requests fan
    /// out before the first finishes, so exactly one request task is kept in flight and
    /// rapid repeat presses all await the same result. A short cooldown after it
    /// completes keeps the same dialog from reappearing on a follow-up press.
    private func requestMicrophoneAndSpeechPermissionsWithoutDuplicatePrompts() async -> Bool {
        ...
        // macOS can briefly report .notDetermined even after the user tapped Allow, so
        // the result of a request that just finished is trusted for a short window.
