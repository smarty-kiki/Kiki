...
/// Claude API client with streaming for progressive text display.
class ClaudeAPI {
    ...
    /// Detects the MIME type from the first bytes — ScreenCaptureKit captures are
    /// JPEG, clipboard pastes are PNG, and the API rejects a media_type that does
    /// not match the image.
    ...
    private func warmUpTLSConnectionIfNeeded() {
        ...
        // The TLS session ticket is host-scoped, so the root host is enough.
    ...
    /// Vision request with streaming. `onTextChunk` is handed the accumulated text so
    /// far each time more arrives, which is what makes the UI update progressively.
    func analyzeImageStreaming(
        ...
        // SSE: one `data: {json}` line per event, terminated by `data: [DONE]`.
