...
一个 macOS 菜单栏伙伴。按住 **Control + Option** 开口问，一个蓝色光标会飞到你屏幕上答案所指的地方并指出来——当模型这么要求时，它还会真的动手：点下去，或者把内容滚出来。
...
滚动默认一屏，要滚更多就写在 label 后面：`[SCROLLDOWN:640,400:消息列表:x3]` 是把那个列表往下滚三屏（1 到 20）。**那个 `x` 不能省**——第二个冒号之后全是元素名字，裸写成 `:3` 会被当成名字的一部分，滚动就悄悄变回一屏。
...
Kiki 不会点击它读起来有破坏性的东西——删除、卸载、格式化、重置、退出、关机、付款、发送——没有辅助功能权限时也完全不动。被拒绝的点击**不会响**：那一声是「点了」的反馈，点在没发生的事上就是在说反话。**滚动和拖拽只受辅助功能权限管**：滚错了往回滚、拖错了拖回去就回去了，所以拦点击的那些字眼不拦它们。
...
| **辅助功能** | 全局 Control+Option 快捷键，以及投出点击和滚动 |
| **屏幕录制** | 抓取屏幕给模型看 |
...
辅助功能权限**不在这份清单里**：没有它 Kiki 照样看得见、答得出、指得准，只是点击和滚动会被拒掉——那和语音那条路完全一样。
...
`-x`/`-y` 是全局屏幕坐标，主屏左上角为原点、y 向下，单位是点。两者必须成对出现，和 `-t` 只能给一组。`-b` 是滚几屏（1 到 20，默认 1），只有四个滚动子命令收它；`--to-x`/`--to-y` 是拖拽的终点，必须成对出现，只有 `drag` 收——给别的子命令传这两个参数是参数错误，不是忽略。
...
滚动、三击和拖拽还各多一条：**正在运行的那个 Kiki 得认识这个手势**。刚重新构建完但没重启 app 时会碰上——它不会把一次滚动、三击或拖拽当成别的动作，而是直接说明并以 `2` 退出。
...
kiki-desktop-agent/                Swift 源码 —— app target
  ...
  CompanionManager.swift             状态机：听写 → 抓屏 → 模型 → 朗读 → 指向
  ...
  ElementClicker.swift               投出鼠标事件与滚动；全 app 唯一会操作机器的地方
...
Kiki 是 Farza 的 [**Clicky**](https://github.com/farzaa/clicky) 的衍生作品，沿用其 **MIT 许可**。
